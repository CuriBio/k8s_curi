...
def upgrade():
    op.execute("INSERT INTO pulse3d_versions (version, state) VALUES ('0.29.0', 'external')")
    op.execute("UPDATE pulse3d_versions SET state='deprecated' WHERE version='0.29.0'")


def downgrade():
    op.execute("DELETE FROM pulse3d_versions WHERE version='0.29.0'")
