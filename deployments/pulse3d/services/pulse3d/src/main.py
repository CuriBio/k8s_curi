...
async def get_upload_route(request: Request, upload_id: str, token=Depends(ProtectedAny(scope=["users:free"]))):
    try:
        user_id = str(uuid.UUID(token["userid"]))
        async with request.state.pgpool.acquire() as con:
...
@app.post("/uploads")
async def create_upload_route(request: Request, details: UploadRequest, token=Depends(ProtectedAny(scope=["users:free"]))):
    try:
        user_id = str(uuid.UUID(token["userid"]))
...
@app.post("/jobs")
async def create_job_route(request: Request, details: JobRequest, token=Depends(ProtectedAny(scope=["users:free"]))):
    try:
        user_id = str(uuid.UUID(token["userid"]))
        logger.info(f"Creating pulse3d jod for upload {details.upload_id} with user: {user_id}")
        ...
        async with request.state.pgpool.acquire() as con:
            job_id = await create_job(con=con, upload_id=details.upload_id, queue='pulse3d', priority=10, meta=meta)
            return {"id": job_id, "user_id": user_id, "upload_id": details.upload_id, "status": "pending", "priority": 10}
