...
def test_get_cfw_from_hw__returns_correct_values(hw_version, channel_fw_version):
    test_cfw_to_hw = {
        "1.0.0": "2.0.0",
        "2.0.0": "2.0.0",
        "3.0.0": "2.0.0",
...
def test_get_previous_sw_version_version__lists_software_installer_objects_from_s3_correctly(mocked_boto3):
    mocked_s3_client = mocked_boto3.client("s3")
    mocked_s3_client.list_objects.return_value = {"Contents": []}

    versions.get_previous_software_version(random_semver())
...
def test_get_previous_sw_version_version__returns_correct_version__when_a_previous_version_exists(
    mocked_boto3,
):
    mocked_s3_client = mocked_boto3.client("s3")
    ...
    def lo_se(Bucket, Prefix):
        return {
            "Contents": [
                {"Key": file_name} for file_name in test_files_in_bucket if file_name.startswith(Prefix)
            ]
        }

    mocked_s3_client.list_objects.side_effect = lo_se

    assert versions.get_previous_software_version("1.1.1") == "1.1.0"


def test_get_previous_sw_version_version__returns_correct_version__when_a_previous_version_does_not_exist(
    mocked_boto3,
):
    mocked_s3_client = mocked_boto3.client("s3")
    ...
    assert versions.get_previous_software_version("1.0.11") is None
...
def test_get_required_sw_version_range__returns_min_software_version_correctly(mocker):
    ...
    mfw_to_sw = {"1.0.0": "1.0.1", "2.0.0": "2.0.1", "3.0.0": "3.0.1"}
...
def test_get_required_sw_version_range__returns_max_sw_version_correctly__when_one_exists(mocker):
    ...
    mocked_get_prev.return_value = max_sw_version = "5.5.5"

    assert versions.get_required_sw_version_range("2.0.0")["max_sw"] == max_sw_version
...
def test_get_required_sw_version_range__returns_max_sw_version_correctly__when_one_does_not_exist(mocker):
    ...
    assert versions.get_required_sw_version_range("3.0.0")["max_sw"] is None
