  ...
  const getWaveformData = async (peaks_valleys_needed, well) => {
    ...
    const checkDuplicates = (well) => {
      ...
      if (wellToUse in originalData.coordinates) {
        ...
        peaksList = peaksList.filter(
          (peak) => dataToCompare[peak][1] >= peakValleyWindows[wellToUse].minPeaks
        );
    ...
    const calculateYLimit = (y1, y2, markerX) => {
      ...
      const x2 =
        ...
        (editableStartEndTimes.endTime - editableStartEndTimes.startTime) / 100;
