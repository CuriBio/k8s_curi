...
async def insert_metadata_into_pg(con, metadata, customer_id, user_id, upload_id, outfile_key, re_analysis):
    ...
    async with con.transaction():
        ...
        if metadata.instrument_type == InstrumentTypes.MANTARRAY:
            try:
                logger.info("Inserting mantarray recording session metadata")
            ...
            try:
                logger.info("Inserting mantarray log metadata")
