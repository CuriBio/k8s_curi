...
def validation_binary(model, criterion, valid_loader, num_classes=None):
    ...
    with torch.no_grad():
        ...
        metrics = {'valid_loss': valid_loss, 'valid_accuracy': valid_jaccard}
        return metrics


def validation_multi(model, criterion, valid_loader, num_classes):
    with torch.no_grad():
        model.eval()
        losses = []
        ...
        print(
            'Valid loss: {:.4f}, average IoU: {:.4f}, average Dice: {:.4f}'.format(valid_loss,
                                                                                   average_iou,
                                                                                   average_dices))
        metrics = {'valid_loss': valid_loss, 'valid_accuracy': average_iou}
        metrics.update(ious)
