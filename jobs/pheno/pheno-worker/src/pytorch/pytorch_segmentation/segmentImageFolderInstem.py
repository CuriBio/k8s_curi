...
arg('--output_mask_type', type=str, default='Overlay', choices=['Mask', 'MaskedImage', 'Overlay', 'IonPath','BoundingBox' ])
...
def mask_overlay(image, mask, alpha, labels, color=(0, 0, 1)):
    ...
    if len(labels)==2:
        ...
        if np.max(image)>256:
            image = image/256
        weighted_sum = cv2.addWeighted(mask, alpha, image.astype(np.uint8), 1-alpha, 0.)
        img = image.copy()
        ...
        for i,l in enumerate(labels[1:]):
            ...
            if i==0:
               ...
               weighted_sum = cv2.addWeighted(msk, alpha, image, 1-alpha, 0.)
               ind = msk[:, :, 0]>0
            else:
...
def patches2image(patches, n, m, center_crop=-1):
    ...
    for i in range(0,nb_rows):
        for j in range(0,nb_cols):
            ...
            mask[-1*i*rows_offset+i*patch_size+ro*i1:-1*i*rows_offset+(i+1)*patch_size-ro*i2, -1*j*cols_offset+ j*patch_size+co*j1:-1*j*cols_offset+(j+1)*patch_size-co*j2] = temp[ro*i1:patch_size-ro*i2,co*j1:patch_size-co*j2]
            pInd = pInd + 1
...
def predict(model, from_file_names, ps, cc, to_path, normalize_patch, output_mask_type,labels, channels):
    ...
    for batch_num, (inputs, paths) in enumerate(tqdm(loader)):
        for i, image_name in enumerate(paths):
            ...
            img_patches = patchImageFolder.image2patches(img, ps, normalize_patch)
            ...
            for p, apatch in enumerate(img_patches):
                ...
                with torch.no_grad():
                    ...
                    if len(labels) ==2:
                        mask_array = (torch.sigmoid(mask_patch).data[0].cpu().numpy()[0]*255 ).astype(np.uint8)
                    else:
...
def main(argsIn):
    ...
    predict(model, file_names, args.patch_size, args.center_crop, output_path, normalize_patch, args.output_mask_type, labels, args.channels)
