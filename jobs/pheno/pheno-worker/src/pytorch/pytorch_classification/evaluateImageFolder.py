...
from .classifyImageFolder import main
...
def main(argsIn):
    ...
    classes, class_to_idx = find_classes(inputFolder)
    ...
    for i, clas in enumerate(classes):
        ...
        if len(os.listdir(subFolder)) > 0:
            ...
            classifyArgs = [
                ...
                args.checkpoint,
                "--precrop-size",
                str(args.precrop_size),
                "--noscale",
                str(args.noscale),
                "--convert2gray",
                str(args.convert2gray),
                "--num-classes",
                str(num_classes),
                "--log-file",
                args.log_file,
            ...
            probs, imageNames, patchProbs, patchNames = main(classifyArgs)
            ...
            for k, prob in enumerate(probs):
                ...
                logging.info(
                    "Filename: {}, true label: {}, prediction: {}".format(imageNames[k], clas, classes[ind])
                )

                # write to csv
                writer.writerow(
                    [imageNames[k] + "," + args.trainorval + "," + clas + "," + ",".join(probList)]
                )
    ...
    if CALC_Z_SCORE:
        ...
        logging.info(
            "Z-Score {} - avg0: {}, std0: {}, avg1: {}, std1: {}, z: {}".format(
                args.trainorval, avg0, std0, avg1, std1, zscore
            )
        )
    else:
        zscore = None
