...
from train_metadata import MetadataModel
import optimPatch
import cv2
...
arg('--classnames', default=None, type=str, help='optional comma-separated classnames, used for cam output (default: None)')
...
def main(argsIn):
    global args, GPU
    global metadata, lenMetadata, all_metadata
    ...
    # parse metadata
    metadata = False
    if '_smeta_' in imagefiles[0] and 'resnet' in args.arch:
        logging.info("Reading metadata from filenames")
        metadata = True
        all_metadata = []
        for fname in imagefiles:
            mdata = re.search("smeta_(.*?)_emeta", fname).group(1)
            params = mdata.split("_")
            all_metadata.append([float(val) for val in params])
        all_metadata = torch.tensor(all_metadata)
        lenMetadata = len(all_metadata[0])
    ...
    if not metadata and args.camfolder is not None and 'resnet' in args.arch and args.mode =="categorical":
        ...
        for i, image_fn in enumerate(imagefiles):
            ...
            if len(image_patches)>0:
                ...
                for p, apatch in enumerate(image_patches):
                    ...
                    if GPU:
                        if metadata:
                            data = all_metadata[i,:].unsqueeze_(0)
                            output = model(patch_tensor.cuda(), data.cuda())
                        else:
                        ...
                        if metadata:
                            data = all_metadata[i,:].unsqueeze_(0)
                            output = model(patch_tensor, data)
                        else:
                    ...
                    if not metadata and args.camfolder is not None and 'resnet' in args.arch and args.mode=="categorical":
                        try:
                            ...
                            outFile = os.path.join(args.camfolder, noextfn+'_patch{:02d}_pred{pred}.png'.format(p, pred=predClass))
