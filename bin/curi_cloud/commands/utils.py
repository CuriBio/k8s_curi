...
def _add_service(deployment_dir, deployment_name, service_name):
    ...
    write_template(
        "tf_output.mako",
        f"{deployment_dir}/{deployment_name}/terraform/{service_name}/output.tf",
        service_name=service_name,
    )
    write_template(
        "tf_service_main.mako",
        f"{deployment_dir}/{deployment_name}/terraform/{service_name}/main.tf",
        service_name=service_name,
    )
    write_template("service_app.mako", f"{service_dir}/src/main.py")
    write_template(
        "00-manifest.mako",
        f"{deployment_dir}/{deployment_name}/manifests/{service_name}-svc.yaml",
        service_name=service_name,
...
def _add_deployment(deployment_dir, deployment_name):
    ...
    if not os.path.isfile(f"{deployment_path}/manifests/{deployment_name}-dep.yaml"):
        write_template(
            "00-deployment.mako",
            f"{deployment_path}/manifests/{deployment_name}-dep.yaml",
            deployment_name=deployment_name,
        )
